...
if __name__ == '__main__':
    ...
    twitter_100 = api.load(modelName)
    ...
    with open('glove-twitter-100-details.csv', 'w', newline='') as csvfile:
        ...
        for i in range(len(questionWord)):
            ...
            if (question[0] not in twitter_100.index_to_key) \
                    or (words[0] not in twitter_100.index_to_key and words[1] not in twitter_100.index_to_key
                        and words[2] not in twitter_100.index_to_key and words[3] not in twitter_100.index_to_key):
                ...
                for j in range(len(words)):
                    if words[j] not in twitter_100.index_to_key:
                        sims.append(0.0)
                    else:
                        sims.append(twitter_100.similarity(question, words[j])[0])
    ...
    with open('analysis.csv', 'a') as csvfile:
        ...
        csvfile.write(modelName + ',' + str(len(twitter_100)) + ',' + str(cLabel) + ','
