...
if __name__ == '__main__':
    ...
    with open('glove-wiki-gigaword-300-details.csv', 'w', newline='') as csvfile:
        ...
        for i in range(len(questionWord)):
            ...
            if (question[0] not in wiki_300.index_to_key) \
                    or (words[0] not in wiki_300.index_to_key and words[1] not in wiki_300.index_to_key
                        and words[2] not in wiki_300.index_to_key and words[3] not in wiki_300.index_to_key):
                ...
                for j in range(len(words)):
                    if words[j] not in wiki_300.index_to_key:
                        sims.append(0.0)
                    else:
                        sims.append(wiki_300.similarity(question, words[j])[0])
    ...
    with open('analysis.csv', 'a') as csvfile:
        ...
        csvfile.write(modelName + ',' + str(len(wiki_300)) + ',' + str(cLabel) + ','
